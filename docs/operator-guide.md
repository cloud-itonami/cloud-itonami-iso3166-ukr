...
2. Register the client's intake: business type, target public function,
   prior filing history in Ukraine if any.
3. Run the advisor in read-only mode against Prozorro tender notices.
4. Compare the checklist against the client's current documentation
   (EDR/EDRPOU registration, QES certificate).
...
- named referral relationship with Ukrainian-licensed counsel or a
...
a working referral relationship with Ukrainian-licensed counsel or a
registered agent for whatever licensed representation the law of
Ukraine requires for actual public-procurement filings.
