(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Ukraine-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Ukraine-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ua-entity?
                                       true` (near-universal for Ukraine
                                       ...
                                       Grounded in Ukrainian legal entity
                                       (EDRPOU) typically required for Prozorro
                                       awards (contracting authorities /
                                       Prozorro).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(EDRPOU/統一資格/Prozorro登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ua-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ua-entity? true`, INDEPENDENTLY verify
  `:has-ua-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ua-entity?` ground truth (most Ukraine public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ua-entity? e))
                 (not (true? (:has-ua-entity? e))))
        [{:rule :ua-entity-missing
          :detail (str subject " はウクライナ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- edrpou-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-edrpou? true`, INDEPENDENTLY check
  `:edrpou-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-edrpou? e))
                 (not (true? (:edrpou-verified? e))))
        [{:rule :edrpou-unverified
          :detail (str subject " はEDRPOU確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ua-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (edrpou-unverified-violations request st)
