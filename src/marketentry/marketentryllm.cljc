(ns marketentry.marketentryllm
  ...
  the UKR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ua-entity?=" (:has-ua-entity? e)
                        " edrpou-verified?=" (:edrpou-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ua-entity? e))
                              (:has-ua-entity? e))
                          (or (not (:requires-edrpou? e))
                              (:edrpou-verified? e)))
                   0.9 0.3)}))
